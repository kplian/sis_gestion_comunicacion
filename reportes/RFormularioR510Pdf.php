...
class RFormularioR510Pdf extends  ReportePDF {
    var $datos_titulo;
    var $datos_detalle;
    var $ancho_hoja;
    var $gerencia;
    ...
    function Header() {
        ...
        $this->Cell(0,5,"REGISTRO DE COMPUTADOR PORTATIL ",0,1,'C');
        //$this->Ln();
        $this->SetFont('','B',9);
        $this->Cell(0,5,"5-R-510/0",0,1,'C');
        $this->Ln(2);
        ...
        $this->MultiCell(15,5,"PANTALLA", 1, 'C', 0, 0, '', '', true);
        $this->MultiCell(15,5,"VIDEO", 1, 'C', 0, 0, '', '', true);
        $this->MultiCell(15,5,"TECLADO", 1, 'C', 0, 0, '', '', true);
        $this->MultiCell(25,5,"PROCESADOR", 1, 'C', 0, 0, '', '', true);
        $this->MultiCell(20,5,"RAM", 1, 'C', 0, 0, '', '', true);
        $this->MultiCell(25,5,"DISPOSITIVO DE ALMACENAMIENTO", 1, 'C', 0, 0, '', '', true);
        $this->MultiCell(20,5,"SISTEMA OPERATIVO", 1, 'C', 0, 0, '', '', true);
        $this->MultiCell(15,5,"ESTADO FISICO", 1, 'C', 0, 0, '', '', true);
        $this->MultiCell(30,5,"ACCESORIOS", 1, 'C', 0, 0, '', '', true);
        $this->MultiCell(20,5,"CODIGO INMOVILIZADO", 1, 'C', 0, 0, '', '', true);
        $this->MultiCell(20,5,"FECHA DE ASIGNACION", 1, 'C', 0, 0, '', '', true);
        $this->MultiCell(20,5,"FECHA DE DEVOLUCION", 1, 'C', 0, 0, '', '', true);
        $this->MultiCell(25,5,"RESPONSABLE DE ASIGNACION", 1, 'C', 0, 0, '', '', true);
    ...
    function imprimirLinea($val,$count,$fill){
        ...
        $conf_par_tablewidths=array(5,25,30,15,15,20,15,15, 15,25,20,25,20,15,30,20,20,20,25,30);
        ...
        $RowArray = array(
            ...
            's4' => $val['modelo'],
            's5' => $val['num_serie'],
            's6' => $val['tamano_pantalla'],
            's7' => $val['tarjeta_video'],
            's8' => $val['teclado'],
            's9' => $val['procesador'],
            's10' => $val['memoria_ram'],
            's11' => $val['almacenamiento'],
            's12' => $val['sistema_operativo'],
            's13' => $val['estado_fisico'],
            's14' => $val['accesorios'],
            's15' => $val['codigo_inmovilizado'],
            's16' => date("d-m-Y", strtotime($val['fecha_inicio'])),
            's17' => date("d-m-Y", strtotime($val['fecha_fin'])),
            's18' => $val['asignador'],
            's19' => $val['observaciones']);
